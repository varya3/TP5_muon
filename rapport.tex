...
\end{tabular}
  ...
  \todo{werte einsetzen, fehler berechnen...sorry bin zu müde etz...}
...
Avec les dates de la mesure de $T=\unit[1000]{s}$ on trouve pour le flux absolu
   ...
   \subsection{Comparaison}
    Le flux vertical de rayonnement cosmique au niveau de la mer est $\Phi_0\approx\unit[90]{\frac{1}{m^2s\cdot sr}} $, cette valeur augmente avec l'altitude car le trajet en air est plus court, donc moins particule ont déjà réagi. \todo{Problem: unsere Werte (die ich ausgerechnet habe...) passen nicht!!!, Frage: oder bleibt er gleich, wweil Muonen kaum reagieren?}.\\
